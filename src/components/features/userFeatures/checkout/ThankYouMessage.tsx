...
const ThankYouMessage = ({ thanks }: { thanks: boolean }) => {
  const [isOpen, setIsOpen] = useState(thanks);
  return (
    <div className="card" onClick={() => setIsOpen(true)}>
      <div
        className={`relative bg-black w-[300px] sm:w-[350px] ${
          isOpen && "group"
        } transition-all duration-700 aspect-video flex items-center justify-center`}
      >
        <div
          className={`transition-all flex flex-col items-center py-5 justify-start duration-300 ${
            isOpen && "-translate-y-16"
          } group-hover:duration-1000 bg-white w-full h-full absolute group-hover:-translate-y-16`}
        >
          ...
          <p className="px-10 text-[10px] sm:text-[12px] text-gray-700">
            It’s so nice that you had the booked our room.
          </p>
          <p className="font-serif text-[10px] sm:text-[12px text-gray-700">
            Wishing you a fantastic day ahead!
          </p>
          <p className="font-sans text-[10px] text-gray-700 pt-5">Flexispace</p>
        </div>
        ...
        <div
          className={`${
            isOpen && "[clip-path:polygon(50%_0%,_100%_0,_0_0)]"
          } tp transition-all duration-1000 group-hover:duration-100 bg-neutral-800 absolute  w-full h-full`}
        ></div>
